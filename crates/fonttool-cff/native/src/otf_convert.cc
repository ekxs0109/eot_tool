...
extern "C" eot_status_t otf_convert_to_truetype_sfnt(
  ...
  hb_blob_t *preflight_blob = nullptr;
  hb_face_t *preflight_face = nullptr;
  ...
  std::fprintf(stderr, "otf_convert: parse start\n");
  std::fflush(stderr);

  preflight_blob = hb_blob_create_or_fail(
      reinterpret_cast<const char *>(font_bytes),
      static_cast<unsigned int>(font_size),
      HB_MEMORY_MODE_READONLY, nullptr, nullptr);
  if (preflight_blob != nullptr) {
    preflight_face = hb_face_create_or_fail(preflight_blob, 0u);
    if (preflight_face != nullptr) {
      std::fprintf(stderr,
                   "otf_convert: preflight hb glyphs=%u upem=%u\n",
                   hb_face_get_glyph_count(preflight_face),
                   hb_face_get_upem(preflight_face));
      std::fflush(stderr);
    }
  }
  ...
  if (sfnt_font_has_table(&source_font, TAG_CFF2)) {
    /* CFF2 variable input is supported through Rust-side instancing. */
  ...
  std::fprintf(stderr, "otf_convert: source parsed tables=%zu\n", source_font.num_tables);
  std::fflush(stderr);
  ...
  std::fprintf(stderr, "otf_convert: hb face ready glyphs=%u upem=%u\n",
  ...
  std::fprintf(stderr, "otf_convert: draw funcs ready\n");
  std::fflush(stderr);
  ...
  std::fprintf(stderr, "otf_convert: outline tasks done\n");
  std::fflush(stderr);
  ...
  std::fprintf(stderr, "otf_convert: outline status ok\n");
  std::fflush(stderr);
  ...
  std::fprintf(stderr, "otf_convert: tt_rebuilder done tables=%zu\n", rebuilt_font.num_tables);
  std::fflush(stderr);
  ...
  std::fprintf(stderr, "otf_convert: copy tables done tables=%zu\n", converted_font.num_tables);
  std::fflush(stderr);
...
cleanup:
  hb_face_destroy(preflight_face);
  hb_blob_destroy(preflight_blob);
