...
| `crates/fonttool-cli/src/main.rs::subset_file(...)` non-OTF subset execution | `fonttool-cli` -> `fonttool-subset` | `crates/fonttool-cli/src/main.rs` -> `crates/fonttool-subset/src/lib.rs` | CLI argument validation and supported non-OTF subset execution are now Rust-owned, including warning emission and `.eot` / `.fntdata` output creation. OTF subset remains deferred. | `cargo test -p fonttool-cli --test subset -q` |
