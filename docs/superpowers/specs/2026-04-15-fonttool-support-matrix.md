...
| `src/table_policy.c` | `fonttool-subset` | `crates/fonttool-subset/src/lib.rs` | Rust now owns the supported table classification matrix, but archived native subset/encode flows still carry legacy warning/output behavior outside the Rust Phase 1 boundary. | `cargo test -p fonttool-subset -q` |
| `src/table_policy.h` | `fonttool-subset` | `crates/fonttool-subset/src/lib.rs` | There is still no Rust-owned C-ABI replacement for native callers that include this header. Phase 1 only requires the Rust table-policy API used by the rewrite crates. | `cargo test -p fonttool-subset -q` |
...
| `crates/fonttool-harfbuzz/src/lib.rs::run_subset_adapter(...)` deferred shellout adapter | `fonttool-harfbuzz` | `crates/fonttool-harfbuzz/src/lib.rs` | Non-OTF subset execution, warning emission, and output writing are still missing from the Rust-owned path. This is Phase 2-owned work, so Phase 1 only requires the no-shellout deferred boundary to stay explicit. | `cargo test -p fonttool-cli --test subset -q` |
...
| `crates/fonttool-cli/src/main.rs::subset_file(...)` non-OTF deferred shellout boundary | `fonttool-cli` -> `fonttool-harfbuzz` | `crates/fonttool-cli/src/main.rs` -> `crates/fonttool-harfbuzz/src/lib.rs` | CLI argument validation is Rust-owned, but non-OTF subset execution and output creation are still missing. This stays Phase 2-owned; Phase 1 requires that the deferred boundary is explicit and shellout-free. | `cargo test -p fonttool-cli --test subset -q` |
...
| CLI `subset` for non-OTF input with `--glyph-ids` | unsupported | rust | yes | Rust still owns argument validation for this path, but execution now stops with `subset execution for non-OTF input remains Phase 2-owned...` instead of shelling out through `fonttool-harfbuzz`. |
...
| `HDMX` behavior during subset (`warning` + drop) | archive-only | legacy | yes | Rust CLI no longer executes subset rebuilds in the forward-supported boundary. This warning/drop behavior currently survives only in archived native subset flows pending Phase 2 replacement. |
...
| `VDMX` behavior during subset (`warning` + drop) | archive-only | legacy | yes | Rust CLI no longer executes subset rebuilds in the forward-supported boundary. This warning/drop behavior currently survives only in archived native subset flows pending Phase 2 replacement. |
