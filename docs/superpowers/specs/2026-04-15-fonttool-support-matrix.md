...
| CLI `subset` for non-OTF input with `--glyph-ids` | unsupported | rust | yes | Rust still owns arg parsing and subset planning, including the supported `.eot`/`.fntdata` decode slice, but execution now stops with `subset execution for non-OTF input remains Phase 2-owned...` instead of shelling out through `fonttool-harfbuzz`. |
...
| Rust runtime entrypoint `fonttool_runtime::convert_otf_to_embedded_font` for static CFF input | unsupported | rust | yes | The Rust API is current, but it now returns `CffError::EncodeDeferredToPhase3` instead of shelling out through `fonttool-cff`. |
...
| Rust WASM crate bridge `fonttool_wasm::wasm_convert_otf_to_embedded_font` for static CFF input | unsupported | rust | yes | Same ownership split as the runtime crate: the Rust API surface now rejects static CFF conversion with `CffError::EncodeDeferredToPhase3` instead of shelling out. |
