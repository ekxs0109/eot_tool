...
static void test_otf_convert_static_cff_matches_with_single_thread_override(void) {
  ...
  parallel_runtime_clear_test_env();
  ASSERT_OK(otf_convert_to_truetype_sfnt(otf.data, otf.length, NULL, &default_font));
  ASSERT_TRUE(parallel_runtime_last_run_task_count() > 1u);
  ASSERT_OK(sfnt_writer_serialize(&single_thread_font, &single_thread_bytes,
                                  &single_thread_size));
  ASSERT_OK(sfnt_writer_serialize(&default_font, &default_bytes, &default_size));
  ASSERT_EQ_SIZE(single_thread_size, default_size);
  ASSERT_TRUE(memcmp(single_thread_bytes, default_bytes, single_thread_size) == 0);

cleanup:
  parallel_runtime_clear_test_env();
  free(default_bytes);
  free(single_thread_bytes);
  sfnt_font_destroy(&default_font);
...
extern "C" void register_otf_convert_tests(void) {
  ...
  test_register("test_otf_convert_static_cff_matches_with_single_thread_override",
                test_otf_convert_static_cff_matches_with_single_thread_override);
