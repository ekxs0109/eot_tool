...
static void test_otf_convert_variable_instance_is_deterministic_across_runtime_modes(void) {
  ...
  ASSERT_OK_CLEANUP(file_io_read_all("testdata/cff2-variable.otf", &otf));
  ...
  ASSERT_OK_CLEANUP(otf_convert_to_truetype_sfnt(otf.data, otf.length, &location, &single_font));
  ...
  ASSERT_OK_CLEANUP(otf_convert_to_truetype_sfnt(otf.data, otf.length, &location, &threaded_font));
  ASSERT_EQ_SIZE_CLEANUP(parallel_runtime_last_run_task_count(), glyph_count);
  ASSERT_EQ_SIZE_CLEANUP(parallel_runtime_last_run_requested_threads(), 4u);
  ...
  ASSERT_OK_CLEANUP(sfnt_writer_serialize(&single_font, &single_bytes, &single_size));
  ASSERT_OK_CLEANUP(sfnt_writer_serialize(&threaded_font, &threaded_bytes, &threaded_size));
  ASSERT_EQ_SIZE_CLEANUP(single_size, threaded_size);
  ASSERT_TRUE_CLEANUP(memcmp(single_bytes, threaded_bytes, single_size) == 0);
