...
| `tests/test_cli.c` | partial | `crates/fonttool-cli/tests/cli_contract.rs` + `crates/fonttool-cli/tests/workspace_cli_smoke.rs` + existing integration tests | Rust now covers top-level help/no-command behavior, unknown command status, decode/encode missing-arg contract errors, successful decode of the supported `font1.fntdata` fixture, decode/TrueType-encode success outside the workspace root without `build/fonttool`, decode of an XOR-obfuscated supported `.fntdata` copy, and the supported non-OTF subset execution slice with rebuilt core tables in supported `.eot` / `.fntdata` outputs. Native-only gaps are now mostly archived success-output text, `.fntdata` obfuscation-flag encode assertions, warning-count/order checks, and thread/env legacy parity. |
...
| `tests/test_cvt_codec.c` | covered | `crates/fonttool-mtx/tests/codecs.rs` | Rust now owns the public `cvt_encode` / `cvt_decode` helper surface plus core delta/range/truncation coverage for the supported codec behavior. |
| `tests/test_glyf_codec.c` | covered | `crates/fonttool-glyf/src/encode.rs` + `crates/fonttool-glyf/src/decode.rs` | Rust now covers detailed `glyf` codec vectors, leading push/code stream edge cases, and decode-side corruption rejection for the supported TrueType path. |
| `tests/test_hdmx_codec.c` | covered | `crates/fonttool-mtx/tests/codecs.rs` + `tests/rust_integration/subset.rs` | Rust now owns the public `hdmx_encode` / `hdmx_decode` helper surface, shared-width decode behavior, roundtrip coverage, corrupt-input rejection, and subset-time drop-with-warning behavior. |
