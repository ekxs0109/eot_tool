...
| `tests/test_parallel_runtime.cc` | archive-only | `crates/fonttool-runtime` tests + `tests/rust_integration/runtime_wasm.rs` | Rust owns the supported runtime semantics; native C ABI/export parity was retired with `src/`. |
