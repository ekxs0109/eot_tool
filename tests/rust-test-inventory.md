...
| `tests/test_table_policy.c` | covered | `crates/fonttool-subset` tests | Rust now owns the supported table-policy classification matrix and subset output drop decisions via `fonttool-subset` unit tests; later encode/subset callers can share the same boundary while native compatibility code remains archival. |
