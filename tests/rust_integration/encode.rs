...
fn encode_can_emit_raw_sfnt_v1_payload() {
    ...
    parse_sfnt(&read_embedded_payload_bytes(&bytes)).expect("raw payload should be a valid SFNT");
}
...
fn assert_roundtrip_preserves_mtx_copy_distance(relative_fixture: &str) {
    ...
    assert_eq!(
        ...
        embedded_font_copy_dist(
            &fs::read(&input_path).expect("original fixture should be readable")
        ),
...
fn encode_pptx_case7_preserves_original_copy_distance_metadata() {
    ...
    let original_bytes =
        fs::read(support::workspace_root().join("build/pptx_case7/ppt/fonts/font1.fntdata"))
