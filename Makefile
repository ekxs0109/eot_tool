...
WASM_DOWNLOAD_CMD ?= curl --fail --location --retry 3 --retry-delay 1
WASM_SHA256_CHECK_CMD := $(shell if command -v shasum >/dev/null 2>&1; then printf '%s' 'shasum -a 256 -c -'; elif command -v sha256sum >/dev/null 2>&1; then printf '%s' 'sha256sum -c -'; fi)
...
$(WASM_HARFBUZZ_ARCHIVE): | $(WASM_THIRD_PARTY_DIR)
	$(WASM_DOWNLOAD_CMD) -o $@.tmp $(WASM_HARFBUZZ_URL)
	mv $@.tmp $@

$(WASM_HARFBUZZ_EXTRACT_STAMP): $(WASM_HARFBUZZ_ARCHIVE) | $(WASM_THIRD_PARTY_SRC_DIR)
	test -n "$(WASM_SHA256_CHECK_CMD)" || { echo "error: need shasum or sha256sum for WASM HarfBuzz checksum verification" >&2; exit 1; }
	printf '%s  %s\n' "$(WASM_HARFBUZZ_SHA256)" "$(WASM_HARFBUZZ_ARCHIVE)" | $(WASM_SHA256_CHECK_CMD)
...
$(WASM_SINGLE_OBJ_DIR)/harfbuzz-subset.o: $(WASM_HARFBUZZ_EXTRACT_STAMP) | $(WASM_SINGLE_OBJ_DIR)
	$(WASM_CXX) $(WASM_CXXFLAGS) $(WASM_COMMON_FLAGS) -c $(WASM_HARFBUZZ_SOURCE) -o $@
...
$(WASM_PTHREADS_OBJ_DIR)/harfbuzz-subset.o: $(WASM_HARFBUZZ_EXTRACT_STAMP) | $(WASM_PTHREADS_OBJ_DIR)
	$(WASM_CXX) $(WASM_CXXFLAGS) $(WASM_COMMON_FLAGS) -pthread -s USE_PTHREADS=1 -c $(WASM_HARFBUZZ_SOURCE) -o $@
